...
<button class="pure-button" style="font-size: 125%;background: rgb(202, 60, 60);color:#EDEBE1;" onclick="poll()">Ask For Poll</button>
    ...
    <?php if(isset($poll)){
        echo "
    <button class='pure-button' style='font-size: 200%;margin:10px;background: rgb(202, 60, 60);color:#EDEBE1;float:left;' onclick='pollresult()'>
      View Poll Result
    </button><div id='pollresult'></div>";
    }
    else 
      echo "
...
function answer(one){
  ...
  var divid = "test"+one;
  var askques= $("#"+divid).text();
  $("#"+divid).load('http://localhost/p/welcome/updateanswerquestion',{question: askques});
    $("#"+divid).fadeOut(600);
...
function pollresult(){
  ...
  $.ajax({
      ...
      //i had to create another test fiddle to overcome the 'same origin' issues
      url: '<?php echo base_url() ?>welcome/pollresult',
      success: function(data) {             
          ...
          $('#mainquestion').fadeOut(100, function() {
            //this is a callback once the element has finished hiding
              ...
              //populate the div with whatever was returned from the ajax call
              $('#mainquestion').load('<?php echo base_url() ?>welcome/pollresult');
              //fade in with new content
              $("#mainquestion").fadeIn(900);
          });
      }
